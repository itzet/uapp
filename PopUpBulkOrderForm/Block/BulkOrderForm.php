 ...
 */
namespace Urjakart\PopUpBulkOrderForm\Block;
 ...
 * Class BulkOrderForm
 *
 * @package Urjakart\PopUpBulkOrderForm\Block
 */
class BulkOrderForm extends Template
{
    /**
    ...
    /**
     * @var \Urjakart\PopUpBulkOrderForm\Block\BulkOrderForm
     */
    private $errors = [];
     ...
     * @return string
     */
    public function getBulkCSRFToken() {
     ...
     * @return string
     */
    private function getMsgHtml($curl, $ref_no, $name, $email, $phone, $qty, $price, $reason, $remark) {
        ...
        $msg .= '<tr><td height="10"></td></tr>';
        $msg .= '<tr><td ' . $style . '>Qty : ' . $qty . '</td></tr>';
        $msg .= '<tr><td height="10"></td></tr>';
        $msg .= '<tr><td ' . $style . '>Expected price : ' . $price . '</td></tr>';
        if(!empty($remark)) {
            $msg .= '<tr><td height="10"></td></tr>';
            $msg .= '<tr><td ' . $style . '>Remark : ' . $remark . '</td></tr>';
        }
        $msg .= '<tr><td height="10"></td></tr>';
        $msg .= '<tr><td> ' . $style . '>Reason for buying : ' . $reason . '</td></tr></table>';
    ...
    private function getClientMsgHtml($ref_no, $name) {
        ...
        $msg .= 'I am Shubham and I will take up your request for bulk order. ';
        ...
        $msg .= '2. Click on Register Button on top right corner of webpage.<br>';
        $msg .= '3. Enter the required details.<br>';
        $msg .= '4. Submit and your account will be created.<br><br>';
        $msg .= 'After that, sit back and relax, I will soon be in touch. </td></tr>';
        $msg .= '<tr><td height="10"></td></tr>';
        $msg .= '<tr><td ' . $style . '>Regards,<br>Subham Kumar<br></td></tr></table>';
     ...
     * @return array
     */
    private function saveData($sub, $name, $phone, $curl, $qty, $price, $email, $reason, $remark, $sku) {

        $return = ['refno' => '', 'error' => ''];
        try {
            $tableName = $this->connection->getTableName('form_popupbulkorder');
            $query = 'select ref_no from ' . $tableName . ' order by id desc limit 1';
            $ref_count = $this->connection->fetchRow($query);
            if (isset($ref_count['ref_no']) && $ref_count['ref_no'] == 'BO005000') {
                $ref_count['ref_no'] = 'BO' . sprintf('%06d', rand(4000, 5000));
            }
            $ref_count = isset($ref_count['ref_no']) ? $ref_count['ref_no'] : 5000;
            $ref_count = explode('O', $ref_count);
            $ref_count = (int)(end($ref_count)) + 1;
            $ref_no = 'BO' . sprintf('%06d', $ref_count);
            $data = [
                'ftitle' => $sub,
                'name' => $name,
                'phone_no' => $phone,
    ...
    private function sendEmail($to, $toClient, $name, $sub, $subClient, $msg, $msgClient, $mUrl, $bcc = '') {
        ...
        try {
            ...
            if ( $mail_us && $mail_client ) {
                $this->messageManager->addSuccessMessage(__('Hi, I have received your bulk order, We will contact you soon. (skumar@urjakart.com| +91-9599-304-982)'));
                if(!empty($_SESSION['backUrl']) && $mUrl=='mbulkorder') {
    ...
    private function sendMessage($name, $phone, $ref_no) {
        ...
        $msg .= '%20have%20received%20your%20bulk%20order%20request%20no.%20';
     ...
     * @return array
     */
    public function validateData($name, $toClient, $phone, $qty, $price, $reason, $remark) {

        if (!(strlen(trim($name)) >= 2 &&
            \Zend_Validate::is(str_replace(' ', '', $name), 'Alpha') &&
            strlen(trim($name)) <= 40)
        ) {
            $this->errors[] = 'Full name must contain alphabets and max 40 characters are allowed.';
        }
        if (!(
        ...
        if (!empty($price) &&
            !(\Zend_Validate::is($price, 'Digits') &&
                $price <= PHP_INT_MAX
            )
        ) {
            $this->errors[] = 'Expected price must be an integer.';
        }
        if (!(\Zend_Validate::is($qty, 'Digits') && $qty >= 1 && $qty <= 9999)) {
            $this->errors[] = 'Quantity must be integer and min 1 and max 9999 allowed.';
        }
        if (!$this->validateReason($reason)) {
            $this->errors[] = 'Reason must be a valid option.';
        }
        if (strlen(trim($remark)) > 65535) {
     ...
     * @return Null
     */
    public function bulkOrderProcess() {
        $this->refNo = null;
        ...
        if($this->getRequest()->getPost('bulksubmit') !== 'bulksubmit'){
           return ;
        }
        ...
        if ( $token && isset($_SESSION['csrf_token']) && in_array($token, $_SESSION['csrf_token'])) {
            ...
            if(!empty($_SESSION['backUrl']) && $mUrl == 'mbulkorder'){$productUrl = $_SESSION['backUrl'] ;}else{$productUrl = $this->getProductUrl();}
            $curl = $productUrl;
            $name = $this->getRequest()->getPost('name');
            $phone = $this->getRequest()->getPost('mobile');
            $remark = $this->getRequest()->getPost('remark');
            $reason = $this->getRequest()->getPost('user_type');
              if(!empty($_SESSION['fsku'])){$sku = $_SESSION['fsku'];} else { $sku = $hsku;}
            $this->validateData($name, $toClient, $phone, $qty, $price, $reason, $remark);
            if ($this->errors) {
            ...
            $ref_no = $this->saveData($sub, $name, $phone, $curl, $qty, $price, $toClient, $reason, $remark, $sku);
            if ($ref_no['error']) {
                $this->messageManager->addErrorMessage(__($ref_no['error']));
                $this->removeToken($token);
                return;
            }
            $msg = $this->getMsgHtml($curl, $ref_no['refno'], $name, $toClient, $phone, $qty, $price,$reason, $remark);
            ...
            $_SESSION['refB'] = $this->refNo;
            $subClient ='Your Bulk Order Request No - '.$ref_no['refno'];
